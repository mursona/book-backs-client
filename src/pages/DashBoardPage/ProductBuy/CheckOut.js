...
const CheckOut = ({ bookedprodut }) => {
  ...
  useEffect(() => {
    ...
    fetch("https://book-back-server.vercel.app/create-payment-intent", {
      method: "POST",
      headers: {
  ...
  const paymentAdd = () =>{
    fetch(`https://book-back-server.vercel.app/addpayment?sellerEmail=${sellerEmail}&product_name=${product_name}&email=${user?.email}`,{
        method : 'PUT',
        headers: {
            "content-type": "application/json",
            authorization: `bearer ${localStorage.getItem("backToken")}`,
  ...
  const paymentAddinfoinbookedCollection = () =>{
    fetch(`https://book-back-server.vercel.app/paymentbooked?sellerEmail=${sellerEmail}&product_name=${product_name}&email=${user?.email}`,{
        method : 'PUT',
        headers: {
            "content-type": "application/json",
            authorization: `bearer ${localStorage.getItem("backToken")}`,
  ...
  return (
    <>
      <form className="w-96" onSubmit={handleSubmit}>
        ...
        />
        <Button className="mt-4" color="pink" variant="gradient" type="submit" disabled={!stripe || !clientSecret || loadding}>Pay Now</Button>
      </form>
      {
        success && <div>
            <p className="text-green-500"> {success} Payment id {trangaction} </p>
        </div>
      }
